...
beforeEach(() => {
  ...
  process.env.PIN2WIN_PAYMENT_NOTIFICATION_EMAIL =
    'operations@pin2wingolf.example, manager@pin2wingolf.example, operations@pin2wingolf.example';
...
test('sends separate idempotent staff and player payment emails', async () => {
  ...
  const result = await sendPaymentConfirmationEmails(input);
  ...
  assert.deepEqual(staffBody.to, [
    'operations@pin2wingolf.example',
    'manager@pin2wingolf.example',
  ]);
  assert.match(staffBody.subject, /^Payment received: \$20\.00/);
  assert.doesNotMatch(staffBody.text, /TEST-CODE/);
  assert.equal(
  ...
  assert.doesNotMatch(playerBody.text, /TEST-CODE/);
  assert.match(playerBody.text, /hides 10 minutes after it is first revealed/);
  assert.equal(
  ...
  await sendPaymentConfirmationEmails({
    ...input,
    checkout: { ...input.checkout, staffEmailSentAt: '2026-09-03T00:00:00.000Z' },
    onAudienceSent: async (audience) => audiences.push(audience),
  });
  assert.equal(requests.length, 3);
  assert.deepEqual(audiences, ['player']);
  assert.deepEqual(JSON.parse(requests[2].body).to, ['player@example.com']);

  requests = [];
  const retryCheckout = { ...input.checkout };
  global.fetch = async (_url, init) => {
    requests.push(init);
  ...
  const retryInput = {
    ...input,
    checkout: retryCheckout,
    onAudienceSent: async (audience) => {
      retryCheckout[`${audience}EmailSentAt`] = '2026-09-03T00:00:00.000Z';
    },
  };
  await assert.rejects(sendPaymentConfirmationEmails(retryInput));
  assert.equal(retryCheckout.staffEmailSentAt, '2026-09-03T00:00:00.000Z');
  await sendPaymentConfirmationEmails(retryInput);
  assert.equal(requests.length, 3);
  assert.deepEqual(requests.map((request) => JSON.parse(request.body).to), [
    ['operations@pin2wingolf.example', 'manager@pin2wingolf.example'],
    ['player@example.com'],
    ['player@example.com'],
  ]);
});
