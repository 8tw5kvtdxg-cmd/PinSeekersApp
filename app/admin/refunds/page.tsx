...
export default async function AdminRefundsPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string; source?: string }> }) {
  ...
  return <AdminShell eyebrow="Payment operations" title="Refund claims" description="Review evidence, record a decision, then explicitly request a verified Square refund. Provider-pending refunds remain pending until Square confirms them.">
    <nav className="my-5 flex flex-wrap gap-4 text-sm font-semibold underline"><Link href="/admin/refunds">Customer claims and reviewed cases</Link><Link href="/admin/reconciliation">Automatic payment alerts</Link><Link href="/admin/refunds?source=automatic">Historical automatic records</Link></nav>
    {automaticHistory && <p className="rounded bg-amber-50 p-4">These records were generated by the previous payment checker, not submitted by customers. They are preserved for audit history. New automatic detections appear under Automatic payment alerts. No refund is issued without explicit administrator action.</p>}
