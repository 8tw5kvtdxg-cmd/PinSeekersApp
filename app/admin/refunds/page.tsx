import { automaticClaimHistoryWhere, customerManagedClaimWhere } from "@/lib/refund-claim-source";
...
export default async function AdminRefundsPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string; source?: string }> }) {
  await requireAdminSession("/admin/refunds");
  const prisma = getPrismaClient();
  const notices = await searchParams;
  const automaticHistory = notices.source === "automatic";
  const claims = prisma ? await prisma.paymentIssueClaim.findMany({
    where: automaticHistory ? automaticClaimHistoryWhere : customerManagedClaimWhere,
  ...
  return <AdminShell eyebrow="Payment operations" title="Refund claims" description="Review evidence, record a decision, then explicitly request a verified Square refund. Provider-pending refunds remain pending until Square confirms them.">
    <nav className="my-5 flex flex-wrap gap-4 text-sm font-semibold underline"><Link href="/admin/refunds">Customer claims and reviewed cases</Link><Link href="/admin/reconciliation">Historical payment alerts</Link><Link href="/admin/refunds?source=automatic">Historical automatic records</Link></nav>
    {automaticHistory && <p className="rounded bg-amber-50 p-4">These records were generated by the previous payment checker, not submitted by customers. They are preserved for audit history. Automatic detection is now disabled. No refund is issued without explicit administrator action.</p>}
    {!automaticHistory && <p className="my-4 text-sm">Customers submit their payment, reason, incident details and supporting evidence. Each submission is saved here immediately and notification emails are queued for the customer and configured staff recipients.</p>}
    ...
    <div className="mt-7 space-y-5">{claims.map(claim => <section key={claim.id} className="rounded-2xl border border-stone-200 bg-white p-6">
      ...
      {claim.systemIssueKey && <p className="mt-2 text-sm font-semibold text-amber-800">Origin: historical automatic detection — not a customer submission</p>}
      ...
      {!claim.refund && !automaticHistory && <form action="/api/admin/refunds" method="post" className="mt-5 grid gap-3 border-t pt-5 sm:grid-cols-2">
